...
| `TenantInvoices` | `/tenant/my-bills` | `Pages/Tenant/TenantInvoices.razor`<br/>`Pages/Tenant/TenantInvoices.razor.cs` | View all invoices for the tenant's lease with filtering by status and pagination. Shows only issued invoices (not drafts) by default. Mobile-friendly responsive layout. Protected by `PolicyNames.RequireTenantRole` authorization. |
| `TenantInvoiceDetail` | `/tenant/my-bills/{id}` | `Pages/Tenant/TenantInvoiceDetail.razor`<br/>`Pages/Tenant/TenantInvoiceDetail.razor.cs` | Detailed view of a specific invoice including line items, payment status, and totals. Includes placeholder for payment tracking and download PDF. Mobile-friendly responsive layout. Protected by `PolicyNames.RequireTenantRole` authorization. |

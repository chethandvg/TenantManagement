# Archu.Web Integration with Aspire AppHost

## Overview

Archu.Web (Blazor WebAssembly) has been successfully integrated into the Aspire AppHost orchestration, enabling seamless development and deployment with automatic service discovery and configuration.

## Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                    Archu.AppHost (Aspire)                    │
│                                                               │
│  ┌───────────────┐  ┌──────────────┐  ┌──────────────┐     │
...
### 1. **AppHost Project** (`Archu.AppHost.csproj`)

Added project reference to Archu.Web:

```xml
<ProjectReference Include="..\Archu.Web\Archu.Web.csproj" />
```
...
- 🔧 Archu.Api (Backend API on dynamic port)
- 👨‍💼 Archu.AdminApi (Admin API on dynamic port)
- 🌐 Archu.Web (Blazor WebAssembly on dynamic port)

**Access the application:**
- **Aspire Dashboard**: http://localhost:15001 (or shown in console)
- **Archu.Web**: Check Aspire Dashboard for the Web app URL
- **Archu.Api**: Check Aspire Dashboard for the API URL
...
If you need to run Archu.Web independently:

```sh
dotnet run --project src\Archu.Web
```

**Note:** 
- You must manually ensure Archu.Api is running on https://localhost:7123
...
Aspire injects these configuration keys into Archu.Web:
...
- [Authentication Implementation](../Archu.Web/AUTHENTICATION.md)
- [ApiClient README](../Archu.ApiClient/README.md)
- [API Documentation](../Archu.Api/README.md)
